...
export function AccountPage() {
  ...
  async function savePassword(event) {
    ...
    try { await changePassword(form.currentPassword, form.password); setForm({ currentPassword: "", password: "", confirmPassword: "" }); setMessage("Password changed successfully."); showToast("Password changed successfully."); }
...
export function AddressPage() {
  ...
  function remove(address) { const next = addresses.filter((entry, index) => index === 0 || addressLabel(entry) !== addressLabel(address)); saveStoredAddresses(user, next.slice(1)); if (addressLabel(booking.address) === addressLabel(address)) update({ address: addresses[0] || null }); showToast("Address removed."); }
...
export function AddressFormPage() {
  ...
  function save(event) { event.preventDefault(); if (!form.line1 || !form.city || !form.state || !/^\d{6}$/.test(form.pincode || "")) return setError("Please fill house number, city, state and a 6-digit PIN code."); const address = { ...form }; saveStoredAddresses(user, [...storedAddresses(user), address]); update({ address }); showToast("Pickup address saved."); navigate("/addresses"); }
...
export function PaymentPage() {
  ...
  async function submitPayment() { setSubmitting(true); setError(""); try { const next = await shopService.submitUpiPayment(id); setData((current) => ({ ...current, ...next, payment: { ...current.payment, ...next.payment } })); showToast("Payment submitted for shop verification."); } catch (err) { setError(err.message); } finally { setSubmitting(false); } }
